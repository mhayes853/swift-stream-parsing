...
public struct JSONParser: ~Copyable {
  ...
  mutating func consumeStringRun<Sink: StreamParseSink & ~Copyable>(
    ...
    while true {
      ...
      if end > i {
        if self.highSurrogate != 0 { throw self.error(.invalidEscape, at: i) }
      ...
      i &+= 1
      if byte == .asciiQuote {
        if self.highSurrogate != 0 { throw self.error(.invalidEscape, at: byteAt) }
        sink.stringEnd()
  ...
  mutating func consumeEscape<Sink: StreamParseSink & ~Copyable>(
    ...
    // leaves the high surrogate lone.
    if self.highSurrogate != 0 { throw self.error(.invalidEscape, at: offset &- 1) }
